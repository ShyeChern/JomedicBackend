...
customerModel.view = function (result) {
    var sql = "SELECT email, password FROM dummytable";
...
customerModel.signUp = function (userId, name, email, password, customerId, icPassportNo, phoneNo, dateOfBirth, question,
    ...
    pool.getConnection(function (err, con) {
        if (err) throw err;
        con.query(sql, [email, userId, email, md5(password), question, answer, customerId, userId, userId, name,
            icPassportNo, dateOfBirth, phoneNo, picture, email], function (err, res) {
                if (err) {
                    con.destroy();
                    result(err, null);
                }
...
customerModel.healthcare = function (result) {
    // var sql = "SELECT DISTINCT h.hfc_name, h.hfc_cd, t.tenant_state_cd, t.longtitude, t.latitude, t.office_photo " +
    //     "FROM jlk_health_facility_old h INNER JOIN jlk_tenant t " +
...
customerModel.healthcareDetail = function (HealthcareId, result) {
    // var sql = "BEGIN; " +
    //     "SELECT t.longtitude, t.latitude, t.tenant_phone_no, w.working_day, t.description, " +
    //     "CONCAT(t.tenant_address1, ', ', t.tenant_address2, ', ', t.tenant_address3) AS address, h.hfc_name " +
    //     "FROM jlk_tenant t INNER JOIN jlk_health_facility_old h " +
    //     "ON t.tenant_id = h.tenant_id " +
    //     "INNER JOIN jlk_working_day w " +
    //     "ON h.tenant_id = w.tenant_id " +
    //     "WHERE h.hfc_cd = ? ; " +
    //     "SELECT AVG(rating) AS rating, COUNT(rating) AS review_count FROM jlk_feedback " +
    //     "WHERE feedback_to IN (SELECT tenant_id FROM jlk_health_facility_old WHERE hfc_cd = ?); " +
    //     "COMMIT; "
...
customerModel.doctorDetail = function (DoctorId, CustomerId, result) {
    var sql = "BEGIN; " +
        "SELECT t.tenant_id, t.tenant_name, t.status, q.qualification_cd, q.field_study, q.university_name, " +
        ...
        "quota FROM pms_duty_roster WHERE hfc_cd=? AND start_date>=CURDATE() ORDER BY start_date LIMIT 7;" +
...
//         "WHERE c.customer_id=? AND m.order_status='active' AND m.receiver_id=?);" +
...
//         "SELECT @user_id, CONVERT_TZ(NOW(), '+00:00', '+08:00'), ?, 'CHAT', CONVERT_TZ(NOW(), '+00:00', '+08:00'), @user_id, ?, ?, '[]', 'active' WHERE NOT @active; " +
//         "SELECT order_no, receiver_id FROM jlk_message_queue m " +
//         "INNER JOIN jlk_customer_acc c ON m.user_id = c.user_id " +
//         "WHERE c.customer_id=? AND m.order_status='active' AND receiver_id=? ;" +
//         "COMMIT;";

//     pool.getConnection(function (err, con) {
//         if (err) throw err;
//         con.query(sql, [CustomerId, DoctorId, CustomerId, OrderNo, DoctorId, 'tbd', CustomerId, DoctorId], function (err, res) {
//             if (err) {
//                 con.destroy();
//                 result(err, null);
//             }
...
//     var sql = "SELECT txn_data FROM jlk_message_queue m " +
//         "INNER JOIN jlk_customer_acc c ON m.user_id=c.user_id " +
//         "WHERE m.order_no = ? AND m.order_status='active' ";
...
// customerModel.sendChatMessage = function (Message, OrderNo, result) {

//     var sql = "BEGIN; " +
//         "SET @message_id = (SELECT JSON_LENGTH(txn_data)+1 " +
//         "FROM jlk_message_queue " +
//         "WHERE order_no=?); " +
//         "UPDATE jlk_message_queue " +
//         "SET txn_data = JSON_ARRAY_APPEND(txn_data, '$', " +
//         "JSON_OBJECT('MessageId', @message_id, 'UserType', 'Customer', 'Message', ?, 'Time', TIME_FORMAT(CONVERT_TZ(NOW(), '+00:00', '+08:00'), '%I:%i %p'))) " +
//         "WHERE order_no=? AND order_status='active'; " +
//         "COMMIT; ";

//     pool.getConnection(function (err, con) {
//         if (err) throw err;
//         con.query(sql, [OrderNo, Message, OrderNo], function (err, res) {
//             if (err) {
//                 con.destroy();
//                 result(err, null);
//             }
...
customerModel.checkReject = function (OrderNo, result) {
    ...
    var sql = "SELECT order_status FROM jlk_message_queue WHERE order_no=? ";
...
customerModel.videoCallEnd = function (CustomerId, ChatDuration, OrderNo, result) {
    ...
    var sql = "BEGIN; " +
        ...
        //here add select doctorid name etc
        "SELECT od.order_no, DATE_FORMAT(od.txn_date, '%d/%m/%Y %H:%i') AS txn_date, od.amount, od.item_desc, t.tenant_name, p.service_name " +
        "FROM jlk_order_detail od INNER JOIN jlk_message_queue m ON od.order_no=m.order_no " +
...
customerModel.makeAppointment = function (CustomerId, DoctorId, Date, Time, OrderNo, result) {
    ...
    var sql = "BEGIN;" +
        ...
        "INSERT INTO pms_appointment (pmi_no, hfc_cd, appointment_date, start_time, txn_date, userid, order_no, status) " +
    ...
    pool.getConnection(function (err, con) {
        if (err) throw err;
        con.query(sql, [CustomerId, DoctorId, Date, DoctorId, Date, CustomerId, DoctorId, Date, Time, DoctorId, OrderNo], function (err, res) {
            if (err) {
                con.destroy();
                result(err, null);
            }
...
customerModel.appointment = function (CustomerId, result) {
    ...
    var sql = "SELECT DATE_FORMAT(a.appointment_date, '%b %d (%a)') AS appointment_date, DATE_FORMAT(a.start_time, '%H:%i') AS start_time, " +
        "a.order_no, t.tenant_name, t.longtitude, t.latitude, CONCAT(t.tenant_address1, ', ', t.tenant_address2, ', ', t.tenant_address3) AS address " +
...
customerModel.cancelAppointment = function (OrderNo, result) {
    ...
    var sql = "BEGIN;" +
        ...
        // "UPDATE pms_appointment SET status='cancelled' WHERE order_no=?;" +
...
customerModel.selfInfo = function (CustomerId, result) {
    var sql = "SELECT p.name, p.picture, AVG(f.rating) AS rating " +
        ...
        "INNER JOIN jlk_feedback f ON f.feedback_to = p.user_id " +
        "WHERE c.customer_id=?";
...
customerModel.currentChat = function (CustomerId, result) {
    var sql = "SELECT m.order_no, t.tenant_id, t.tenant_name, p.picture, q.field_study FROM jlk_message_queue m  " +
        "INNER JOIN jlk_customer_acc c ON m.user_id=c.user_id " +
        "INNER JOIN jlk_tenant t ON m.receiver_id=t.tenant_id " +
        "INNER JOIN jlk_user_profile p ON t.user_id=p.user_id " +
        "INNER JOIN jlk_qualification q ON t.tenant_id=q.tenant_id " +
...
customerModel.history = function (CustomerId, result) {
    // DATE_FORMAT(om.txn_date, '%d/%m/%Y %H:%i') AS txn_date
